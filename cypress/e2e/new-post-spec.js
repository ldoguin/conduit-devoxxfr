...
describe('New post', () => {
  ...
  it('can edit an article', () => {
    ...
    cy.get('[data-cy=title]').should('have.value', 'my title');
    cy.get('[data-cy=title]').clear().type('a brand new title');
    cy.get('[data-cy=title]').should('have.value', 'a brand new title');
    cy.get('[data-cy=publish]').click()
  ...
  it('adds a new post', () => {
    ...
    cy.request( `${apiUrl}/api/articles?limit=10&offset=0`)
      .its('body')
      .should(body => {
